...
In addition to OIDC (for authentication), CP4AIOps Foundational Services can also connect directly to LDAP for user/group search and role mapping. This guide follows the IBM Foundational Services LDAP connection guide.
### Configure LDAP via the CP4AIOps Management Console
1. Log in to the CP4AIOps Management Hub: `https://<cp4aiops-route>/zen/`
...
4. Configure the following settings for the new LDAP connection:
...
5. Click `Test connection` → expect green success indicator
6. Click `Create connection` to save the new LDAP connection
...
1. On the OCP Bastion VM, run the following command:
...
1. Launch web browser, navigate to the CP4AIOps Login page and login as `cpadmin`
...
| Specify LDAP connection | Choose `ldap` | This is the LDAP connection created in earlier steps |
...
| Settings | Value |
|---------|-------|
| Well-known URI | `https://<keycloak.example.com>:<keycloak-port>/auth/realms/{realm}/.well-known/openid-configuration` |
| Client ID | `cp4aiops-client` |
...
We will assign LDAP user with Roles in CP4AIOps.
...
1. Launch new Private/Incognito Firefox window navigate to the CP4AIOps Login page
...
1. Configure SevOne Observer
2. Add menu option in topology to launch SevOne DI for discovered devices
