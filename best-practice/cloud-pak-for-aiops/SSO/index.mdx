...
6. You will be directed to the SevOne DI dashboard for the selected device, indicating that Single Sign-On (SSO) has been successfully configured between SevOne and CloudPak for AIOps.

<div align="center">
![sevone cross launch](./images/sevoneDI-cross-launch.png)
</div>

7. To further validate the configuration, log in to the Keycloak administration console as an administrator. Under `sevone-realm` → `Sessions`, you should see that user `alice` has active sessions for both `cp4aiops-client` and `sevoneDI`, confirming successful authentication across the two applications.
