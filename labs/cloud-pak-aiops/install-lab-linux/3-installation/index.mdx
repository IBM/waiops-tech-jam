...
[root@aiopsc1 ~]# 
```
Run the following command to discover your what your additional disks are.
```
...
[root@aiopsc1 ~]# 
```
In this case, you can see the primary control plane node `aiopsc1` has two additional 200G disks: `sdc` and `sdd`. When you configure your local volumes, one will be allocated for the storage of AIOps application data (`APP_STORAGE_PATH`), and one will be allocated for the storage of the Kubernetes platform data (`PLATFORM_STORAGE_PATH`).
...
Once you have identified your additional disks, follow the steps on the link below to set these volumes up:
...
After you have successfully configured your volumes, you should see something like the following for your control plane nodes:
...
After you have successfully configured your volumes, you should see something like the following for your worker nodes:
