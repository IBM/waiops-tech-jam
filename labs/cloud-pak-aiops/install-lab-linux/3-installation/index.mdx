...
The product documentation for this installation process is published [here](https://www.ibm.com/docs/en/cloud-paks/cloud-pak-aiops/4.12.0?topic=linux-online-installation).
...
* You access the environment via the bastion host (`bastion-gym-lan`)
* You set up the cluster from the bastion host via the primary control plane node host (`aiopsc1`)
* Your browser on the bastion host connects to the load balancer host (`aiopslb`)
* The load balancer host load-balances incoming connections to the three control plane nodes
...
ssh jammer@aiopsc1 "sudo -i bash -c 'echo \"$PUBLIC_KEY\" >> ~/.ssh/authorized_keys' && echo \"SSH key added to aiopsc1 successfully\""
```
...
export SSH_OPTIONS="-o StrictHostKeyChecking=no"
for host in aiopsc1 aiopsc2 aiopsc3 aiopsw1 aiopsw2 aiopsw3 aiopsw4 aiopsw5 aiopsw6; do
    ssh -t $SSH_OPTIONS jammer@$host 'sudo dnf install -y lvm2'
...
for host in aiopslb aiopsc1 aiopsc2 aiopsc3 aiopsw1 aiopsw2 aiopsw3 aiopsw4 aiopsw5 aiopsw6; do
    ssh -t $SSH_OPTIONS jammer@$host 'sudo dnf update -y'
...
The script in this section will connect to each one of the control plane nodes, identify the spare disk volumes, then format and mount them, ready for the installation of AIOps. The control plane nodes use one partition as the storage for of K3S platform data and a second one for the storage of AIOps application data.

:::note

The control plane nodes will have two additional disks as described above. The worker nodes however will only have one additional disk for the storage of AIOps application data. They do not need the K3S platform storage directory like the control plane nodes do.

:::

The process to set up your disks is:
- Identify the disks on the host
- Create the physical volumes
- Create the volume groups
- Create the logical volumes in the volume groups
- Format the logical volumes
- Create the local directory mount points
- Add the new file system to the file system table (`/etc/fstab`)
- Mount the logical volumes onto the local directory mount points

Run the following command as the default **admin** user from the **bastion host** to set up your application storage disk and your platform storage disk on all your control plane node hosts:


```sh title="Host: bastion-gym-lan"
export SSH_OPTIONS="-o StrictHostKeyChecking=no"
for HOST in aiopsc1 aiopsc2 aiopsc3; do
  ssh -t $SSH_OPTIONS jammer@$HOST 'sudo bash -s' << 'EOF'
    echo "$(hostname) - Finding the 200G disks..."

    # 1. Find the 200G disks that are not partitioned
    ...
    # 2. Exit if suitable disks are not found
    if [ -z "$APP_DISK" ]; then
      echo "$(hostname) - ERROR: No suitable 200G disk for APP_DISK found. Aborting..."
      exit 1
    fi

    if [ -z "$PLATFORM_DISK" ]; then
      echo "$(hostname) - ERROR: No suitable 200G disk for PLATFORM_DISK found. Aborting...."
      exit 1
    fi
    ...
    # 6. Create logical volumes on the volume groups
    lvcreate aiops -n aiops -l 100%FREE
    lvcreate aiopspl -n aiopspl -l 100%FREE

    # 7. Format the new logical volumes
    ...
    # 9. Add the new filesystem directories to /etc/fstab
    ...
    # 10. Mount the filesystems onto the mount points
...
EOF
  ...
  echo "$HOST disk setup complete."
done
```

As you successfully configure your volumes, you should see something like the following in the output for each of your **control plane node** hosts:
...
Run the following command as the default **admin** user from the **bastion host** to set up your application storage disk on all your worker node hosts:
...
for HOST in aiopsw1 aiopsw2 aiopsw3 aiopsw4 aiopsw5 aiopsw6; do
  ssh -t $SSH_OPTIONS jammer@$HOST 'sudo bash -s' << 'EOF'
    ...
    # 1. Find the 200G disk that is not partitioned
...
<details>
<summary>What if my installation hangs?</summary>

:::warning Only perform these steps if your installation fails

You should not perform the steps in this sub-section unless you suspect your installation has hung.

:::

The installation should take under an hour to complete. If your installation takes longer than an hour, it may be stuck. This can happen if some pods get to an "Error" state, even when replacement pods subsequently start successfully. The presence of the Errored pods can cause the installation not to complete successfully and appear to hang.

Leave the installation terminal window running and open a **new** terminal window session on the **bastion host**. Run the following command as the default **admin** user in the new terminal window session on the **bastion host** to check for any Errored pods:

```sh title="Host: bastion-gym-lan"
. /home/admin/aiops_var.sh
ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} kubectl get pods --all-namespaces | grep Error
```

If you have any, you will see output that looks like the following:

```
connector-orchestrator-86c5645685-pvstn                           1/1     Running                       0               1h12m
connector-orchestrator-86c5645685-t2tz8                           0/1     Error                         0               1h12m
connector-orchestrator-86c5645685-tznpf                           1/1     Running                       0               1h13m
```

If you have any, run the following command as the default **admin** user in the new terminal window session on the **bastion host** to delete any Errored pods:


```sh title="Host: bastion-gym-lan"
ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} kubectl get pods --all-namespaces | grep Error | awk '{print $2 " --namespace=" $1}' | xargs ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} kubectl delete pod
```

After the Errored pods have been deleted, the installation running in your other terminal window should then progress.
...
do
  ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} ssh ${TARGET_USER}@${WORKER_NODE} aiopsctl cluster node down --role worker --app-storage=${APP_STORAGE_PATH}
done
```
...
do
  ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} ssh ${TARGET_USER}@${CP_NODE} aiopsctl cluster node down --role control-plane
done
```
...
This concludes the installation of the IBM Cloud Pak for AIOps on Linux VMs.
