...
Run the following code block on the **bastion host** to enable this:
...
Run the following command on the **bastion host** to create a completely new SSH key which will be used for the AIOps installation onto the AIOps cluster node hosts:
...
Finally, run the following commands on the **bastion host** set up the `root` user on `aiopsc1` to use the new SSH private key to connect to the other AIOps cluster nodes:
...
You will now be able to SSH from the **bastion host** `bastion-gym-lan` to `aiopsc1`, and from `aiopsc1` to any other node in the cluster, as the `root` user, without a password - eg. `ssh root@aiopsw1`.
...
Run the following commands on the **bastion host** to install **lvm2** on each host in the cluster:
...
Next, run the following commands on the **bastion host** to update **all** hosts and reboot them one-by-one:
...
Run the following command block on each of your **control plane** hosts as the `root` user to discover your what your additional disks are for each one:
...
Following on from the previous step, run the following command block on each of your **control plane** hosts as the `root` user to set your storage environment variables according to the names of the additional drives:
...
Finally, run the following commands on each **control plane** host to create the partitions and verify that they have been created correctly:
...
Run the following command block from the **bastion host** to set up your application storage disk on all your worker node hosts:
...
After you have successfully configured your volumes, you should see something like the following in the output for each of your **worker node** hosts:
...
Connect to the **load balancer** host:

```sh title="Host: bastion-gym-lan"
ssh jammer@aiopslb
```

Next, run the following commands on the **load balancer** host to install **HAProxy**:
...
Next, run the following commands on the **load balancer** host to update the HAProxy configuration file, enable HAProxy, and start the HAProxy service:
...
Finally, run the following command on the **load balancer** host to validate your configuration by checking the status of the HAProxy service:
...
Your VM environment is now ready to begin the installation.

First, create an environment variable file on the **bastion host** called `aiops_var.sh` with the following content:
...
- Check the cluster storage performance (optional)
- Run the AIOps install command

### 3.8.1 Install the K3S Kubernetes cluster

First, run the following command on the **bastion host** to source the environment file you created:

```sh title="Host: bastion-gym-lan"
. ./aiops_var.sh
```

Next, run the following commands from the **bastion host** to install `aiopsctl` and initialise the K3S Kubernetes cluster on the primary control plane host:
...
You may have to run the `ssh` commands above one at a time if it does not successfully execute them as a block. This can be a quirk of the virtual desktop lab environment being accessed via a browser.

:::

Next, run the following command block from the **bastion host** to install `aiopsctl` and initialise the K3S Kubernetes cluster on the other two control plane hosts:
...
Finally, run the following command block from the **bastion host** to install `aiopsctl` and initialise the K3S Kubernetes cluster on the worker node hosts:
...
After the setup has completed, run the following commands on the **bastion host** to check the status of the cluster:
...
Run the following command on the **bastion host** to patch the network settings on all cluster member hosts:
...
### 3.8.3 Check the cluster storage performance (optional)

If you have time, you can optionally test the performance of your cluster storage. This step instructs the cluster to pull a storage performance testing image from IBM and then run a series of read and write actions against the cluster storage. At the end of the test, it will give you its results and let you know if there are any potential performance issues. The test can take **up to 10 minutes** to complete.
...
### 3.8.4 Install AIOps to the cluster

Run the following command on the **bastion host** to install AIOps:
...
After the installation has completed, run the following command on the **bastion host** to verify the installation:
...
First, run the following command on the **bastion host** to obtain the AIOps login credentials:
...
Next, run the following command on the **bastion host** and add the the following entries to your local hosts file:
