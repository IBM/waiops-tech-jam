...
The AIOps installation process connects to the various hosts as the `root` user so we will need to enable *PermitRootLogin* on each of the AIOps cluster nodes. The first step to enabling this is to update each host's SSH configuration to allow logins as the `root` user. Run the following code block to enable this:
...
Run the following command on the **bastion host** to store the local public SSH key on `bastion-gym-lan` in the environment variable `PUBLIC_KEY` and then add it to the `root` user's `authorized_keys` file on `aiopsc1`. This will allow passwordless connection between `bastion-gym-lan` and `aiopsc1`.
...
Run the following command on `bastion-gym-lan` to create a completely new SSH key which will be used for the AIOps installation onto the AIOps cluster node hosts:

```bash title="Host: bastion-gym-lan"
ssh-keygen -t rsa -f ~/.ssh/aiops-cluster -N "" -C "AIOps Cluster Key"
```

Run the following command on the **bastion host** to store the new public SSH key in the environment variable `PUBLIC_KEY` and then add it to the `root` user's `authorized_keys` file on all the AIOps cluster nodes *except* for `aiopsc1`.
...
Next, set up the `root` user on `aiopsc1` to use the new SSH private key to connect to the other AIOps cluster nodes:
...
You will now be able to SSH from `bastion-gym-lan` to `aiopsc1`, and from `aiopsc1` to any other node in the cluster, as the `root` user, without a password - eg. `ssh root@aiopsw1`.
...
Run the following commands on `bastion-gym-lan` to install **lvm2** on each host of the cluster:
...
Finally, run the following commands on `bastion-gym-lan` to update **all** hosts and reboot them one-by-one:
...
Alternatively, run the following commands in separate terminal windows in parallel on your bastion host to update **all** hosts and reboot them at the same time:

```sh title="Host: aiopslb aiopsc1 aiopsc2 aiopsc3 aiopsw1 aiopsw2 aiopsw3 aiopsw4 aiopsw5 aiopsw6"
ssh root@aiopsc1
yum update -y
shutdown -r now
```
...
Connect to each of your AIOps cluster nodes as the `root` user:
...
Now you have identified your additional disks for each host, log in to each host and complete the following steps to set them up.

On each **control plane** node, set your environment variables according to the names of the additional drives - example below:
...
You can use either one of your additional drives for application storage and the other for platform storage. The above example uses `/dev/sdc` and `/dev/sdd`. Take care that you enter the correct drive names. They may be different on each control plane node host.

:::

On each **control plane** host, run the following commands to create the partitions and verify that they have been created correctly:
...
After you have successfully configured your volumes, you should see something like the following on your **control plane** hosts output:
...
```sh title="Host: aiopsw1 aiopsw2 aiopsw3 aiopsw4 aiopsw5 aiopsw6"
DISK_ONE=$(lsblk -o NAME,SIZE,TYPE,MOUNTPOINT | awk '
  $2 == "200G" && $3 == "disk" {
    cmd = "lsblk -n /dev/" $1 " | wc -l";
    cmd | getline lines;
...
On each **worker node** host, set your environment variable according to the name of the additional drive - example below:
...
You should have one additional drives for application storage on your worker node hosts. The above example uses `/dev/sdc`. Take care that you enter the correct drive name. It may be different on each worker node host.

:::

On each **worker node** host, run the following commands to create the partition and verify that it has been created correctly:
...
After you have successfully configured your volume, you should see something like the following on your **worker node** hosts output:
...
Connect to the **load balancer** host and change to the `root` user:

```sh title="Host: bastion-gym-lan"
[admin@bastion-gym-lan ~]$ ssh jammer@aiopslb
[jammer@aiopslb ~]$ sudo -i
[root@aiopslb ~]#
```

Run the following command on the **load balancer** host `aiopslb` as the `root` user to install `HAProxy`:

```sh title="Host: aiopslb"
install -y haproxy
```

Apply the `haproxy.cfg` file as-is (no changes are necessary) by running the following command:
...
Validate your configuration by checking the status of the `haproxy` service:
...
Don't worry if you see errors in the status output at this point. This is because the AIOps end-points don't exist yet.

:::

## 3.7: Prepare AIOps environment file

Your VM environment is now ready to begin the AIOps deployment.

Create an environment variable file on the bastion host called `aiops_var.sh` with the following content:
...
```sh title="Host: bastion-gym-lan"
[admin@bastion-gym-lan ~]$ . ./aiops_var.sh
```

Run the following commands from the **bastion host** to install `aiopsctl` and initialise the K3S Kubernetes cluster across the AIOps cluster nodes:
...
After the setup has completed, run the following command to check the status of the cluster:
...
Next, run the following command from `bastion-gym-lan` to update the network settings on **all** hosts:
...
Finally, run the following command from the **bastion host** to install AIOps:

```sh title="Host: bastion-gym-lan"
ssh ${TARGET_USER}@${CONTROL_PLANE_NODE} aiopsctl server up --load-balancer-host="${LOAD_BALANCER_HOST}" --mode "${DEPLOY_TYPE}"
```

The deployment itself should take up to 60 minutes to complete.

After the installation has completed, run the following command from the **bastion host** to verify the installation:
...
If you encounter an error during installation and the installation fails, follow the [uninstallation steps here](https://www.ibm.com/docs/en/cloud-paks/cloud-pak-aiops/4.11.1?topic=linux-uninstalling), then restart the installation from the beginning of [this section](/waiops-tech-jam/labs/cloud-pak-aiops/install-lab-linux/installation/#38-install-aiops).
...
Run the following command on the **bastion host** and add the the following entries to your local hosts file:

```
[admin@bastion-gym-lan ~]$ sudo vi /etc/hosts
...
192.168.252.9 aiops-cpd.aiopslb
192.168.252.9 cp-console-aiops.aiopslb
```

Next, open a Firefox web browser session on your bastion host and go to the AIOps login page.
...
Since we are using self-signed certificates, your browser window will give you certificate security warnings. Simply accept and continue several times to get to the AIOps login page. A bookmark to the AIOps login page is also available in the bookmarks toolbar.

:::

Run the following command on the **bastion host** to obtain the AIOps login credentials:
...
Use the credentials given to log in to the console.
