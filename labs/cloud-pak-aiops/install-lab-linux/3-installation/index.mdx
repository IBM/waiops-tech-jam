...
### 3.4.2 Update the cluster hosts (check with instructor)

This step updates the operating system of all cluster hosts to ensure they are fully up-to-date with the latest patches. While it is very important to do this in a real-world deployment, it is not essential in order to complete this lab. Also, it will require significant downloads and take a considerable amount of time, therefore please check with the instructor before completing this step in a lab.

:::important

Do not run this step unless told to by the lab instructor.

:::

To fully patch the cluster host operating systems, run the following commands as the default **admin** user on the **bastion host** to update **all** hosts and reboot them one-by-one:
...
### 3.8.3 Check the cluster storage performance (check with instructor)

In a real deployment, it is prudent to test the performance of your storage to ensure it will perform adequately. Poorly performing storage is frequently the root cause of many issues experienced in production environments.
...
If completing this lab in an IBM organised session, please ask your instructor first whether or not to proceed with these performance checks. Given the demanding nature of the checks this step carries out, it may place too much pressure on the lab infrastructure if everyone runs this at the same time.
...
If your installation completes successfully, skip to section [3.9: Log in to the Cloud Pak for AIOps console](/waiops-tech-jam/labs/cloud-pak-aiops/install-lab-linux/installation/#39-log-in-to-the-cloud-pak-for-aiops-console).

:::note

If you encounter an error during installation and the installation fails, follow the [uninstallation steps below in section 3.8.5: Uninstall AIOps](/waiops-tech-jam/labs/cloud-pak-aiops/install-lab-linux/installation/#385-uninstall-aiops-if-installation-fails-check-with-instructor), then restart the installation from the [beginning of section 3.8: Install AIOps](/waiops-tech-jam/labs/cloud-pak-aiops/install-lab-linux/installation/#38-install-aiops) to re-run the installation steps. Please check with the instructor before running the uninstallation steps.

:::

### 3.8.5: Uninstall AIOps if installation fails (check with instructor)

The virtualised lab environments can occasionally experience latency and other intermittent connectivity issues. This can sometimes interrupt connectivity between the cluster members, which can cause the installation to fail. If this happens, you will need to uninstall the cluster members and re-run the installation.

:::important

Check with the instructor before completing this section.
