...
With an LDAP connection created, we can add users from the LDAP directory to the Cloud 
...
From the bastion VM, run the commands listed below to obtain the LDAP connection information
...
* If you are not already logged-in, log-on to the IBM Cloud Pak for AIOps console using the admin credentials obtained previously.
...
right icon (Cloud Pak switcher) and select **Automation** to get back to 
the home page of the Cloud Pak for AIOps.
* From the navigation menu, click **Administration** > **Access control**.
...
You have now logged in as the new **sre1** user with the **Administrator** role. You should see the welcome page as shown below:
