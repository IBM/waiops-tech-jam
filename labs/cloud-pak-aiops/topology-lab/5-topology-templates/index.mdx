...
- On the right side, in the search box: **Search for a resource to get started** type **router1** 
  and hit Enter
...
Click on the **Resource group templates** link on the top. You will see your
new template:

![](images/resourcegroup.png)


Click on the **Resource management** link on the top, select the **Resource groups** tab and search for **ROUTER-** , you will see
the following under the **Resource groups** tab:

![](images/search.png)

Click on the name **ROUTER-router1**, you will be able to view the
...
as they have been merged by the Merge Rule defined previously.
...
This is called **topological event correlation** and is one of the correlation modes in addition 
...
topology **tagsRule** as discussed in the previous chapter, or by including the tags parameter 
...
Click on the **Resource group templates** link on the top, filter by **tag template** and you will see your
saved template:

![](images/tag-template.png)

Click on the **Resource management** link on the top, select the **Resource groups** tab and search for **tag-group**, you
will find the new group created by the tag template:

![](images/tag-group.png)

If you click on the name **tag-group**, you will see the resources in the topology group:
...
A dynamic group template builds multiple groups of resources that are similar to the 
...
For example, you can use a dynamic group template to create dynamic groups of resources, 
...
The Dynamic group template is probably the most versatile of all the topology group
...
Create a new file called **application2.txt** by running the following command in the **Terminal** window to open the text editor
...
Use the steps described in the previous chapter to import this file (**application2.txt**) using a 
new File Observer job to ingest the topology data.

After running this last Observer job, you should see a total of 12 resources which 2 have been merged, as shown below:

![](images/12resources.png)



Now lets create the dynamic group template. In the **Resource management** page, click on the **Resource group templates** icon in the top-right.
...
- Enable **Correlate event groups on topologies from this template**
  to correlate events across these resources
- In the search box **Search for a resource to get started**, type **computer1000** and hit Enter
...
- You can optionally add tag to the list of starting seeds if you want to restrict the items
...
In the **Preview resource groups** section, you will see that the dynamic template builder has found two groups of
resources that match the *recipe*. The first group resources were in the original application resources data (**application.txt**). The second group resources were added
by the Observer job we just run (**application2.txt**). In summary, we defined a dynamic template that creates groups for every compute resource 
and will include any one hope resource that has the attribute *runsOn*. 
...
As token group templates apply rules only to resources that are received via observer jobs, the
...
- For **Tokens** type **${name}** and click on **Add**. This is the value of the name property which in our resource data will be *Intel* or *AMD*.
- Expand the **Conditions** section
  ...
  - For **Resource types** select **cpu** and click on **Add**. This is important as we only want to group **cpu** type resources and nothing else. 
...
resources are in the **infrastructure** resource data file, please rerun the **infrastructure** 
observer job to actually apply the rules in this group template.
