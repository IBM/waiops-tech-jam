...
- In the **Estimated cost per minute for service disruption**, type **750**.
  This numerical value, represents the estimated disruption cost (in local
  currency) for the business per minute if this application is down. This
  information is used on the **AIOps insights** dashboard.
