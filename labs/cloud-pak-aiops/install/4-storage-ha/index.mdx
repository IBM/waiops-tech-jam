---
title: 4. Storage, HA and FIPS Considerations
...
The Cloud Pak for Watson AIOps requires persistent storage classes that support the RWO (read-write-once) and RWX (read-write-many) access modes.
...
:::note
If you require your Cloud Pak for Watson AIOps environment to support the Federal Information Processing Standards (FIPS), there are storage
...
As the Cloud Pak for Watson AIOps is used to monitor a client's high value work loads, its important to deploy the solution in a way that
...
* An individual pod failure (#1): Each pod the Cloud Pak for Watson AIOps includes health check endpoints. When a pod reports it is not healthy, OpenShift will 
terminate it, and start a new instance.

* An Openshift cluster node failure (#2): When the Cloud Pak for Watson AIOps is installed in Production/HA mode, 
...
When FIPS support is enabled, the Cloud Pak for Watson AIOps uses cryptographic modules that are compliant with 
...
for the transmission of data. Traffic inside the Cloud Pak for Watson AIOps boundary is still secure, as traffic 
...
for the IBM Cloud Pak foundational services that are used by the Cloud Pak for Watson AIOps.
...
FIPS enabled environment. You also cannot backup the Cloud Pak for Watson AIOps from a non-FIPS enabled environment and 
restore it into a FIPS enabled environment.
:::

To enable FIPS support, the following links describe the tasks required for the OpenShift cluster and Cloud Pak for Watson AIOps installation:
...
* [Installing Cloud Pak for Watson AIOps tasks required for compliance](https://www.ibm.com/docs/en/cloud-paks/cloud-pak-watson-aiops/4.1.0?topic=fips-enabling-support-cloud-pak-watson-aiops#install_tasks)
