...
(note, if you don't see the above login screen when opening up Firefox, click on
the "IBM Dashboard Application Services Hub" bookmark in the bookmarks bar).
...
utilization, for example). -"details:DB_SAN1" is a more specific component of
the node "details", and indicates metrics for that particular sub-component (in
this case, the disk DB_SAN1).
...
Searching for aribitrary metrics allows you to view the behaviour of metrics
