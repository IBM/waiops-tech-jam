...
Keycloak is embedded within the Concert installation and runs as a pod. The admin password is stored in an environment variable under **ibm-solis-embedded-keycloak** pod.
...
2. List all pods under the Concert host to verify that the **ibm-solis-embedded-keycloak** pod is running:
...
3. Retrieve the Keycloak temporary admin username and password from the **ibm-solis-embedded-keycloak** pod environment variables:

```bash title="Host: concert"
# Get the default Keycloak admin username
kubectl exec -it -n platform-hub $(kubectl get pod -n platform-hub -l component=ibm-solis-embedded-keycloak \
    -o jsonpath='{.items[0].metadata.name}')   -- env | grep KC_BOOTSTRAP_ADMIN_USERNAME
```

```bash title="Host: concert"
# Get the default Keycloak admin password
kubectl exec -it -n platform-hub $(kubectl get pod -n platform-hub -l component=ibm-solis-embedded-keycloak \
    -o jsonpath='{.items[0].metadata.name}')   -- env | grep KC_BOOTSTRAP_ADMIN_PASSWORD
```
...
[jammer@concert ~]$ kubectl exec -it -n platform-hub $(kubectl get pod -n platform-hub -l component=ibm-solis-embedded-keycloak \
    -o jsonpath='{.items[0].metadata.name}')   -- env | grep KC_BOOTSTRAP_ADMIN_USERNAME
...
:::note
The Keycloak bootstrap admin username and password are randomly generated during Concert installation. You must retrieve them from the **ibm-solis-embedded-keycloak pod** environment variables as shown above.
:::
...
5. On the Concert welcome page, select the **Resilience** option on the right and select **Skip** in the next page:
...
2. Logout from the Concert UI by clicking on your username in the top right corner and selecting **Logout**.
...
**Problem**: The `kubectl exec -it -n platform-hub $(kubectl get pod -n platform-hub -l component=ibm-solis-embedded-keycloak \
    -o jsonpath='{.items[0].metadata.name}')   -- env | grep -E 'KC_BOOTSTRAP_ADMIN_USERNAME|KC_BOOTSTRAP_ADMIN_PASSWORD'` command fails or returns an error.
...
- Verify that KC_BOOTSTRAP_ADMIN_USERNAME and KC_BOOTSTRAP_ADMIN_PASSWORD exist from : `kubectl exec -it -n platform-hub $(kubectl get pod -n platform-hub -l component=ibm-solis-embedded-keycloak \
    -o jsonpath='{.items[0].metadata.name}')   -- env | grep -E 'KC_BOOTSTRAP_ADMIN_USERNAME|KC_BOOTSTRAP_ADMIN_PASSWORD'` command output, refer to **Section 3.5**
