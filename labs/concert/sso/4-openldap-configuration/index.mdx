...
- **ou (Organizational Unit)**: Logical grouping of related directory entries for organizing entries
...
Next, we will create user accounts and groups on the LDAP. Please leave the OpenLDAP host session open for the next steps.
...
### 4.3.2: Create Users LDIF File

Let's create an LDIF file to add the users with a command that generates hashes automatically.

1. On the OpenLDAP host, perform the following step:

```bash title="Host: bluebox"
cd /opt/openldap/ldap-config
```

2. Create the `create-users.ldif` file with the following content:
...
EOF
```
The `create-users.ldif` file contains **slappasswd** commands to generate hashed passwords for a supplied string for each user.

### 4.3.3: Add Users to LDAP

Do LDAP add to add users:
...
### 4.3.4: Verify Users Creation

Do LDAP search to verify users were created:
...
# numEntries: 3
```
### 4.3.5: Test User Authentication

Use `ldapwhoami` command to verify that users can authenticate with their passwords:
...
[jammer@bluebox ldap-config]$ ldapwhoami -x -D "uid=bob,ou=users,dc=example,dc=com" -w Bob@123
dn:uid=bob,ou=users,dc=example,dc=com

[jammer@bluebox ldap-config]$ ldapwhoami -x -D "uid=charlie,ou=users,dc=example,dc=com" -w Charlie@123
dn:uid=charlie,ou=users,dc=example,dc=com
```
...
Create the `create-groups.ldif` file with the following content:
...
Do LDAP search to verify groups were created:
...
1. Do LDAP search to verify group membership for Alice was created:
...
2. Do LDAP search to verify group membership for Bob was created:
...
3. Do LDAP search to verify group membership for Charlie was created:
...
|----------|----------|-------|
| alice | Alice@123 | corporate_it_staff |
| bob | Bob@123 | hr_staff |
| charlie | Charlie@123 | sales_staff |
