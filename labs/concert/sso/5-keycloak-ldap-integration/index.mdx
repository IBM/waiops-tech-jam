...
![Create Realm](images/keycloak-create-concert-realm.png)
...
Now that the `concert-realm` is active, let's create the OIDC client that Concert will use to authenticate users via Keycloak. <br/>
...
4. Enter the following values:
   ...
   - **Always display in UI**: Toggle to **OFF**

![Create Client](images/keycloak-create-client-values.png)
...
3. Leave the following check boxes unselected :
   ...
   - **OIDC CIBA Grant**
4. Set **Require DPoP bound tokens**: Toggle to **OFF**
5. Click **Next**.

![Create Client Capability](images/keycloak-create-client-capability.png)
...
:::important
The `*` wildcard in Valid redirect URIs must be included to allow Concert to redirect to any path under its domain.
:::

2. Click **Save**.

![Create Client Login](images/keycloak-create-client-login.png)

3. Click **Save** on the summary screen.

![Create Client Summary](images/keycloak-create-client-summary.png)
...
1. **Details** tab shows:
   ...
   Under `User metadata`, the `LDAP_ENTRY_DN` attribute value indicates the user was imported from OpenLDAP
