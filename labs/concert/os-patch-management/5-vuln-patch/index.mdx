...
2. On the **Create Authentication** screen:
   - **Name**: `concert_API`
   - **Service**: **API Key**
   - **Header Name**: `Authorization` (first part of **Concert Request Header** from credentials file. Do not include the semicolon or the space after it, only the word `Authorization`)
...
3. In the following steps, do not select more than one workflow at a time when downloading.
4. Search for **Create_Change_Request_For_Remediation_Action** and download the `.zip` file.  
5. In **Concert Workflows → Workflows**, click **Import**, then select `Create_Change_Request_For_Remediation_Action.zip`.
6. Verify that the workflow appears in the **Workflows** list.

Next, repeat the same steps for **Monitor_Remediation_Action_Status** and **Remediation_Master** and ensure that all workflows 
...
* Click on **View details** to view the security advisories. Concert automatically ingests the security advisories and 
correlates them with the VM scan to understand which security advisories need to be applied.

Please notice, the current status of this particular OS Patch is **Created**, Concert has correlated the VM 
scan with security advisories to understand that an automatic remediation option is available.
...
Add the required authentications to the workflow:
...
Select **Concert** → **Vulnerability** → **Auto-Remediate** and click on the **Refresh** icon. 
...
- **In Progress** to proceed with the OS patching remediation
...
4. Click the **Update** button in the top-right to save the changes.
...
2. In the **Start** block of the workflow, update the following field mappings:
   - **concert_config** → `ibmconcert/concertConfig` — created in Section 5.2.1
   - **snow_auth** → `ibmconcert/snowAuth` — created in Section 5.2.3
...
After running the workflow, you should see the *scheduled* message in the logs. The Concert Action called **Patch** changed it 
status to **Scheduled** and displays the date that the **Remediation_Master** workflow is set to run.
...
periods. We will bypass the scheduled date and manually trigger the **Remediation_Master** workflow to run.
