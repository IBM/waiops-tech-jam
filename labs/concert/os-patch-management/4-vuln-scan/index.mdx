...
2. On the **Create Authentication** screen:
   ...
   - **Private Key**: Copy full private key from `rhel_vm_private_key` in credentials file. DO NOT copy any extra spaces at the beginning or end.
...
configure it to use the authentications you created, and run it to perform a vulnerability scan on the RHEL VM.
...
3. In **Concert Workflows** → **Workflows**, from the top-right corner, click on the 3-dot icon and select 
**Create folder**. Name the folder `SCAN_WORKFLOWS` and click **Create**. 
Click on the newly created SCAN_WORKFLOWS folder to open it and click **Import** → select `vulnerability_scan.zip`.
4. Verify the new workflow appears as `RHEL_SCAN_OSCAP_GRYPE`.
...
- VM scan results in Event Log with CVEs discovered from the scan
...
Click on the **Runtimes** tab (click on the **Refresh** icon) and note that there is a new runtime called **demo-apps** of Type **host** with an 
IP 192.168.252.33. Click on **demo-apps** and scroll down to view the vulnerabilities found by the workflow you just ran.
...
Navigate to **Dimensions → Vulnerability** (click on the **Refresh** icon) and note that there are vulnerabilities discovered from the scan.
...
Select the **Auto-remediate** tab and click on the **Refresh** icon. Note that there is a new remediation action called **Patch**. Concert’s auto-remediation feature automatically detects and fixes security vulnerabilities
...
These actions are automatically submitted for approval through a ServiceNow incident.
