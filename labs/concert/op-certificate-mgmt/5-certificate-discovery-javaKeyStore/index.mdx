...
Let's configure user variables values for **Linux_Keystore_Cert_Discovery** Workflow by using the Authentications created in the previous section. 


:::note
For each default value shown below, make sure it begins and ends with a double quote (").
:::

* Click on the **Workflows** tab on the top and select the **Workflows tile** 
* Click the **certificatesDiscovery** folder
* Click the **Linux_Keystore_Cert_Discovery** workflow. You will now confirm that the Workflow input parameters are using the
authentications you created previously.

    * Under **keystorePath** name, click **Eye** icon as shown below to reveal the text, for Default value, confirm it's `"/opt/tomcat/ssl/server.jks"`

    ![](images/cw-jks-certdiscovery-keystorepath.png)

    * Under **env_name** name, for Default value, confirm it's `"development"`. This refers to the environment that you created in the Lab Preparation section
    * Under **linuxAuth** name, for Default value, confirm it's `"ibmconcert/Ansible-apacheTomcat"` 
...
:::note
In a customer environment, the **Linux_Keystore_Cert_Discovery** Workflow should be configured to 
run automatically as a **job** through a workflow job scheduler. In this lab exercise, however, it is executed manually to demonstrate how it works.
:::
...
* Click the **Run** button for **Linux_Keystore_Cert_Discovery** Workflow
...
* Click on the **Resilience** tab on the top. 
* On the left side menu, select **Dimensions -> Operations**
...
* You will get into **Keystore entries UI** for a Certificate after performing the previous action.

![](images/concert-certificate-keystore-entries-ui.png)

* Click on Back on the top navigation to get back to the previous page. 

Click on the 3 vertical dots at the right of the certificates. You will see the option to 
update some certificate attributes or delete the certificate (don't make any change).

![](images/concert-certificates-3vertical-dots.png)


### 5.3.3: Label the GitHub Issues for Certificate Renewal

In GitHub, you will see the two GitHub issues created automatically by Concert Automation Rules. 
...
Now, using the Ticket/Issue link on the certificate, we will label the issue that belongs to the expired certificate as **Approved** for renewal.

* Click the twistie indicated by the red arrow on the **expired certificate**. Again, make sure you select the expired cert.
* Scroll down and click on the link under the Ticket section to open the Issue created for this expired certificate.

![](images/expired-cert.png)

* On the Issue page, click on **Labels** on the right and check the **Approved** label. You should see the **Approved** label added to the issue as shown below:

![](images/approved-issue.png)

* Just to recap, we have labeled the issue that belongs to the expired certificate as **Approved**. This means that 
we have approved the renewal of this certificate. We will see how it gets renewed in the next section.
...
## 5.5: Get the Certificate Metadata via Concert API
...
From the Bastion SSH, run the following **curl** command. 
Use the credentials file to copy the command below and update the following values:
  - Replace the value after **search=** with the Certificate Serial Number that belongs to the expired certificate from the Concert Operations/Certificates page.
