...
:::note
For each default value shown below, confirm that begins and ends with a double quote (").
:::

* Click on the **Workflows** tab on the top and select the **Workflows tile** 
* Click on the **certificateRenewal** folder
* Click **Linux_Keystore_Cert_Renewal** workflow. You will now confirm that the Workflow input parameters are using the
authentications you created previously plus other configuration information.

    * Under **ansibleAuth** name, for Default value, confirm it's `"ibmconcert/Ansible-apacheTomcat"`
    * Under **passwordMap** name, for Default value, confirm it's `"ibmconcert/CertConfigData"`
    * Under **caOrSelfSigned** name, for Default value, confirm it's `"ca"`
    * Under **caServerAnsibleAuth** name, for Default value, confirm it's `"ibmconcert/Ansible-cfssl"`
    * Under **caType** name, for Default value, confirm it's `cfssl`
    * Under **tomcatServerAnsibleAuth** name, for Default value, confirm it's `"ibmconcert/Ansible-apacheTomcat"`
    * Under **sFtpsrcFilePath** name, for Default value, confirm it's `"/opt/cfssl/cfssl-pki/all-certs/fullchain.pem"`
    * Under **sFtpdestFilePath** name, for Default value, confirm it's `"/tmp/fullchain.pem"`
    * Under **keyStoreFileName** name, for Default value, confirm it's `"server.jks"`
    * Under **gitHubCredentials** name, for Default value, confirm it's `"ibmconcert/GitHub"`
    * Under **gitHubOwner** name, for Default value, **STOP, here you have to type your GitHub username** `"<replace with your GitHub username>"`
    * Under **gitHubRepo** name, for Default value, **STOP, here you have to copy or type** `"concert-operations-lab"`
    * Under **gitHubIssueState** name, for Default value, confirm it's `"open"`
    * Under **gitHubIssueLabel** name, for Default value, confirm it's `"approved"`
    * Under **concertAuth** name, for Default value, confirm it's `"ibmconcert/ConcertAPIKey"`
    * Under **commonSSHcfssl** name, for Default value, confirm it's `"ibmconcert/SSH-cfssl"`
    * Under **commonSSHTomcat** name, for Default value, confirm it's `"ibmconcert/SSH-apacheTomcat"`
...
## 6.3: Manually Run the Renewal Workflow

Now we will run the certificate renewal workflow which:

* will see what issues has been label as **approved**. Remember we did this in the previous section
* collect the certificate information from the issue
* renew the certificate using the private CA
* install the new certificate
* restart Apache Tomcat
* remove the expired certificate metadata from Concert

While on the **Linux_Keystore_Cert_Renewal** workflow, click on the **Run** button. 

After the run finish, verify the logs. You will see run log similar to the screenshot below. The run log shows that the workflow 
completed successfully for the certificate with serial number value.
...
The github issue for the certificate that has not expired yet is still **Open** and has no changed.
...
## 6.4: Performing the Certificate Discovery Again
...
* Click on the **Resilience** tab on top and select **Dimensions -> Operations** on the left side. Make sure to refresh the page 
to see the updates. 

* On the **Certificates** tab, you will see total number of Certificates in the **development** environment is 3,
  where :
...
## 6.5: Get Certificate Metadata via Concert API
...
From the Bastion SSH, run the **curl** command, after replacing the following **two** values:
  - Replace the value after **search=** with the actual Certificate Serial Number obtained from the Concert Certificate UI with expiry date **+ 2 months** from present date
...
curl -sk \
  ...
  --header 'content-type: application/json-patch+json' | jq
```
...
## 6.6: Confirm the Certificate in Java Keystore has a New Expiration Date
...
## 6.7: Make the Firefox Browser to Trust the Certificates
