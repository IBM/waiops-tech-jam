...
In this lab you will be installing Turbonomic platform (highlighted in yellow)
in your **AIOps** OpenShift cluster.
...
1. Go to your OpenShift AIOps Console UI by either selecting the bookmark **OCP
   Dashboard** or navigating to
...
1. Deployment will begin for all the Turbonomic pods. Depending on the
   ...
   **Example output only (Do not copy and paste into your terminal):**
   ...
   The pods **server-power-modeler** & **suspend** may be in a ImagePullBackOff
   state, these can be safely ignored.
...
1. Open the Turbonomic UI in your browser by using either the bookmark called
   **Turbonomic** or get the Turbonomic UI route after the deployment completes:
