...
into the HTML pages. In a containized environment, you would need to rebuild and
deploy the containers. ![](images/websiteConfig2.png)
...
that you can retrieve it later. ![](images/websiteConfig.png)
...
by issuing the following commnand:
...
Install the **Robot Shop** by issuing the following helm command. You'll need to
replace <EUM Key\> with the **key** value that you obtained in section 3.2 from
the Instana user interface and <Reporting URL\> is the **reportingURL** you
obtained in section 4.2.

<span style={{color: "red"}}>
  Note: When you specify the **Reporting URL**, do not include the trailing
  slash ( / ).
</span>
...
Example: **helm install robot-shop --set openshift=true --set
eum.key=m-IMxZYFQyW_n7Xo0VRjrw --set eum.url=http://<your server IP\>:2999 -n
robot-shop helm**
