...
A new dialog containing some options and javascript will open. Leave the default options
as is and scroll down to **Add tracking script**.
...
the **reportingURL**. Make a copy of these values now by copying the code block to a text document.

![](images/website-config-marked.png)

Select **Go to website dashboard**
...
:::warning Important Workaround

There is currently a bug in the Robot Shop application where a trailing slash is stripped from the reporting URL. Until a fix is merged upstream we will modify the Robot Shop web pod to fix this issue by running the following code block:

```sh title="Host: demo-apps" {/* backslash for correct indentation in file */}
# 1. Find the robot-shop web pod name
export POD_NAME=$(kubectl get pods -n robot-shop -l "service=web" -o jsonpath='{.items[0].metadata.name}')

# 2. Wait for pod to be ready
kubectl wait --for=condition=ready pod/$POD_NAME -n robot-shop --timeout=300s

# 3. Wait until the file exists and is accessible
until kubectl exec $POD_NAME -n robot-shop -- test -f /usr/share/nginx/html/eum.html 2>/dev/null; do
    echo "Waiting for eum.html to be available..."
    sleep 2
done

# 4. Fix the trailing slash bug
kubectl exec $POD_NAME -n robot-shop -- \
    sed -i "s|/eum')|/eum/')|g" /usr/share/nginx/html/eum.html
```
...
The result should look similar to: `robot-shop.apps.CLUSTER_DOMAIN`. In our case
...
### Robot Shop Self Signed Certificate Warning
...
![live-last-minute](images/live-last-minute.png)
