...
Once all pods are in a running state you can continue.

Open a new Firefox tab and select the **Robot Shop** bookmark, or if pasting the
route into firefox make sure you specify http instead of https. You will see the
Robot Shop application.

![](images/stans-robot-shop.png)

:::warning Robot Shop Bug in OpenShift Deployments

From the Robot Shop webpage you can press F12 to bring up the developer tools
and select the **Network** tab. As you click around the app you will notice the
failing EUM beacons.

There is a bug in Robot Shop when installed on OpenShift that causes the
trailing slash of the reporting URL to be stripped on install. Without the slash
the EUM data will not be correctly proxied into Instana and therefore no End
User Monitoring details will be shown in Instana.

The workaround we can use for this lab is to manually add the trailing slash
back into the reporting URL with the following command ran from the Bastion:
