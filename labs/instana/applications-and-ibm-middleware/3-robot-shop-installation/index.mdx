...
From the Instana GUI, on left side navigation, select **Websites & Mobile Apps**. Then, click the **Add Website** link.

![](images/add-website.png)

You'll see a dialog where you can enter a Website Name(If it's the first time you 
open the Website & Mobile Apps page, you won't have any existing website configured, 
then you'll directly see this dialog). Enter `Robot Shop` as the website name and 
...
A new dialog containing some options and javascript will open. Leave the options as is.

In a more typical application, you would follow the instructions and copy/paste the javascript into the website's `<head/>` tag.

:::info

Note: For Apache and IBM HTTP Servers, 
you do not need to update the HTML pages. 
You can configure the Instana agent where the Apache HTTP Server is present, 
to automatically inject the JavaScript agent into the website to enable website monitoring. 
Refer to [Website automatic instrumentation](https://www.ibm.com/docs/en/instana-observability/current?topic=websites-automatic-instrumentation) 
for more details.
...
the unique variables. There are two parameters within the javascript that we will
need to copy and use when we do the install. You will need the **key** and the
**reportingURL**. Either copy those parameters now or leave this dialog open so
...
We will install Robot Shop on the Demo Applications Cluster in the **demo-apps** vm.
...
From the Bastion host, open a new terminal window by selecting
...
We will use **Helm**, a package manager for Kubernetes to install Robot Shop. First install Helm by running the following commands:
...
Finally, we want to expose the Robot Shop UI. K3s includes a Traefik Ingress Controller that we can use to create a new Ingress for the app. Run the following to create the ingress:
...
to **Websites & Mobile Apps** > **Robot Shop** you will not see any EUM data displayed in Instana.

![No Data](images/no-data-robot-shop.png)

The reason this is happening is because the Instana EUM reporting URL uses HTTPS `https://instana.techzone.lan/eum/` and the browser is not able to verify the Self Signed Certificate used by Instana in this lab.

To fix this first issue we need to manually accept the Self Signed Certificate, from Firefox select the bookmark **EUM TLS** and accept the certificate by selected **Advanced...** and then **Accept the Risk and Continue**.
