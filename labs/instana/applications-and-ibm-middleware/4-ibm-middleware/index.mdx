...
The **Quote of the Day** application has been installed for you on the **Apps
SNO** cluster.
...
The following should be completed from the **bastion host**
(_admin@bastion-gym-lan_, open a new tab if you are unsure)

:::

Ensure you have opened a new tab in the terminal and are on the Bastion host.

Issue the **oc get pods** command against the **qotd** namespace/project.
...
If you aren't already, become root user on mqace node:
...
There are few configuration files in the parent _etc_ directory, but most of the
...
In addition, here is a blog that provides additional information on configuring the sensor:
[link](https://community.ibm.com/community/user/blogs/hou-fang-zhao/2022/05/24/usefule-tips-for-ace-sensor-configuration)
...
In addition, the Instana engineering team has created a script that will setup all
of the required permissions within MQ.  The script can be found here:  
[link](https://github.com/instana/content/tree/main/ace-setup-mq)
...
Zone that you specified when installing the OpenShift monitoring Agent.
...
the _availability-zone_ to something that makes sense for the demo environment:
**App**. This VM will be zoned with other Apps.
