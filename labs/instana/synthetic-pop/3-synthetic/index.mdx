...
If th pod failing, execute the following command to reduce CPU requests to be able to schedule the pod:
...
a few minutes, change time the frame to a longer period, for example
`last 30 minutes` and refresh the browser page.
...
On the dialog that opens, you will see two options under **API**
test. Choose **API Simple** and click the **Select** button:

![](images/addSynthetics3.png)

Scroll down for a new dialog prompting for the REST API **GET** URL. Enter the following URL:
...
Then, scroll down to Locations and select your **on Instana Cluster** location. This allows you to choose the
location(s) where you want your synthetic script to execute. In the lab, you
only have 1 location that you can select, so we choose `on Instana cluster` and click
**Add location**:

![](images/SSL-certificate-location.png)

The next step allows you to specify the scheduling interval for your synthetic
test. Drag the time slider to 1 minute. Then, scroll down:
...
In the **Create Synthetic Test** dialog, select **API** and **API Script test** click the
**Select** button:

![](images/SyntheticScript.png)

Scroll down to Script section and click **Add Script**.
Paste the following JavaScript code into the **script** text field as shown below:
...
Scroll down, ensure to select the `on Instana cluster` location. Instana will run the
...
Give the test a meaningful name and description, then scroll down. We will
use values:
