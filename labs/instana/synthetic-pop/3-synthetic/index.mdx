...
Before we can create any synthetic tests, we must first setup a synthetic "Point of Presence" or "PoP server" to playback our synthetic tests.
...
We will be installing the Synthetic Point of Presence into the OpenShift (OCP) cluster. In order to do that, we'll need access to a CLI will access to the OCP cluster.
...
You can copy/paste the yaml from the lab guide into the the Instana "core" object.
