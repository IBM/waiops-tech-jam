...
- A **Dynamic** host agent downloads the latest set of sensors from
  ...
  repository.  In addition, it eliminates the need for every Agent to have internet access to the repositories.
...
Instana provides the ability to setup **self monitoring** of the **Standard Edition**
...
Instana backend server is being monitored.
...
will show up within the Instana UI:
...
For Kubernetes based installations, you can specify the same content that would go 
into the configuration.yaml file, but the mechanism is different.  See the product documentation
for Operator and yaml based installations.  For helm installs, you use an **agent.configuration_yaml**
