...
install Instana components on the other hosts. To do this we need to:

1. Create a new ssh key
2. Add the public key to the root user on the other hosts
3. Copy the private key to the **instana-0** host
4. Move the key to root's .ssh directory and set permissions
...
still operating as the **root** user.

:::danger Ensure you are on the right host and your keys are working before
continuing

1. Ensure you are on the correct host. If you are unsure run
   `hostname && whoami` to verify.
   ...
   If you don't you should open a new tab, run `ssh jammer@instana-0` and
   `sudo -i` to become the root user.

2. Ensure you are able to connect to **instana-1** and **instana-2** as the root
   user using the newly created cluster key. Run the following:
