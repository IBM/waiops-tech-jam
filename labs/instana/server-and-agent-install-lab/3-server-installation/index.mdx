...
- **Self Hosted Classic Edition** Install both the Instana backend and the
  ...
  deployment does not support multi-tenancy.  Note: **Classic Edition** is not
...
upgraded every 2 weeks. With the most recent version of **Self-Hosted Standard Edition**,
...
Instana version fairly current. For example, plan for upgrades every 2 months.
...
Note: While 16 cores and 64 Gig of RAM is enough for a POC environment, if you enable 
...
that need to be opened for the Instana Server for an on-prem deployment. The ports for SaaS are slightly different.

In addition, online installations need access to a number of repositories that are documented here for reference:  
...
Update the package list and install the Instana **stanctl** command.
The **stanctl** is used to install Instana and perform other administrative tasks.
...
- We are skipping preflight checks as the environment does not have the storage
  ...
  The installation below, uses a set of optional installation flags.  If you are interested
  in seeing all of the available installation flags, you can type:

  ```sh
  stanctl up --help
  ```

Now, execute the installation command by copying and pasting the text from below.
...
The installation will take approximately 20 minutes, but depends on the performance of your server.
...
The command will list the available updates. Use the arrows on your keyboard to move up and download
to select the version that you want to install.  For example, move the arror to **Install Instana version: 3.287.582-0**
and press **Enter** to upgrade the backend.
...
deployments, customers will want the **multi-node Standard Edition** or **Self-Hosted Custom Edition**. 
**Multi-Node Standard Edition** will increase scalability.  **Self-Hosted Custom Edition** provides more
