...
If you are not on the **Installing Instana Agents** page select the **"Stan"**
robot icon in the upper left corner and click the **"Deploy Agent"** button near
...
will be one pod for each worker node in your target cluster. This SNO instance
...
```sh
oc logs <POD NAME> -c instana-agent -n instana-agent --kubeconfig kubeconfig-apps
```
