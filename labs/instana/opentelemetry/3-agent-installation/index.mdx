...
:::info This script contains all the necessary commands to install the Instana
agent on your Kubernetes cluster using Helm. :::
...
:::caution Please note the below example does not contain real values. The
fields like key, endpoint, etc. have been replaced with [REMOVED] value. You
...
use specific commands to check the status of the deployed resources. Use kubectl
get pods -n instana-agent to list all the pods in the instana-agent namespace
...
:::tip All pods should be in RUNNING state, all of them should also have READY
1/1 values as explained on the picture above. :::
