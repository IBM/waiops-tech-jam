...
on-prem). For on-premises installation, Instana offers multiple options:
...
- **Kubernetes deployment** with the cloud native backend components running in
  ...
  or open a [SWAT Request](http://ibm.biz/automation-swat) request.
...
:::caution A note on copy & paste

Be mindful when copying and pasting commands that extra characters or new lines
are not copied by accident. Remove any extra characters or new lines before
executing the command.

:::

From the Bastion host (named **Guacamole**), open a terminal window by selecting
...
Finally, ensure the required packages are installed for SMTP configuration later
in this lab:
...
Instana requires port 80 so ensure there are no processes currently running on
port 80.
...
Instana server. In this environment it is **192.168.252.60**
...
use CA signed certificates, this is the step where you would specify the
...
- If you forget the password, you can use the following command to reset the
  admin@instana.local password:
  `instana configure admin -p <new_password_for_admin>`
...
Open a firefox browser and navigate to `https://192.168.252.60` or you can
select the **Instana** bookmark if you imported the bookmarks.
...
It will take a little while for the configuration changes to take effect.
[Read section 2-7](#27-upgrading-the-instana-server) while the configuration
...
There are no steps to perform for this section but we want you to know the
process for performing an upgrade.
