...
section, you will find guidence about configuring and using Instana with the
...
<span style={{color: "red"}}>
  {" "}
  This section is for informational purposes only. In your lab environment, these
...
Note: Typically, these steps are performed by the MQ and ACE admins, so you
don't need to know the step by step procedure. But, you do need to tell the
MQ/ACE admins what the monitoring requirements are.
...
> **&#9888; HINT: In the commands listed, BK1 is the Integration Node name and
> would be different if the Integration Node had a different name**
...
<span style={{color: "red"}}>
  The steps in this section have already been performed in your environment. The
  instructions found here are for informational purposes.
</span>
...
Note: Typically, these steps are performed by the MQ and ACE admins, so you
don't need to know the step by step procedure.
...
Note: Even though the TAR files for ACE and MQ look similar, they are unique for
...
**------------IMPORTANT------------** yaml files are very strict about
indentation. Each line within the file must be indented by 0, 2, 4, 6, etc.
spaces. Do not use tabs and do not indent by odd numbers of characters. Make
sure that all of the characters line up vertically within the text. If you see a
line that is commented out and you need to uncomment it, just remove the single
"#" character.
...
Note: The markdown documentation only shows 9 rows of text, but there are more
rows. You can either click the **Show more** button in the bottom right, or you
can click on the **Copy** button in the top right corner.
...
conect to in order to query the performance KPIs. The username of **mqm** is the
admin username for MQ, but the sensor does not require administrative
...
file to specify some key proccesses on this ACE/MQ server. Seach within the
...
Note: There is also a section in the configuration.yaml file where you can
specify processes that you want to ignore. Search for **com.instana.ignore**.
...
MQ monitoring are working propererly. If they are not, come back to this section
and double check your configuraion.yaml settings and check that there are no
