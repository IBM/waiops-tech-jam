...
If MQ and ACE is running the output will look like this:

![ace-mq-status](images/ace-mq-status.png)

Care fully check the output and compare it to the screenshot above. If either of
the above are **not in a running state** for you execute the following script:
...
Once complete you can run the status check again and confirm MQ and ACE are
running.
