...
In this section we'll show you how to setup an Alert Channel. Later, in the Administrative section, we'll use that Alert
Channel to get notifications when an Event triggers.

:::info

In this lab Instana has already been installed with SMTP configured. If you would like to review when that was configured you can [view the Instana installation in the Server & Agent Installation lab](https://ibm.github.io/waiops-tech-jam/labs/instana/server-and-agent-install-lab/server-installation/#35-installing-instana).

There is also an information section giving details on [how to configure SMTP post installation](https://ibm.github.io/waiops-tech-jam/labs/instana/server-and-agent-install-lab/server-installation/#36-setting-up-smtp).

:::

From the Instana UI select the **Settings** gear in the left side navigation. Then, select **Alert Channel**
...
- Enter the email address to notify. Notice that there is an **Add Email** link
  on the right side if you want to add multiple Email addresses. For this lab
  we could use `mqadmins@example.com`.
...
Check for the test email by opening the Firefox bookmark called **Mail** in a new tab
...
Scroll down to the **Scope** section. You'll see that there are number of options to determine which
...
You can try out some of the options, but keep in mind that analytics based thresholds
will require 5 days of data and your system has only been running for a few
...
Here is an example from an app that has been running longer. When you adjust the sensitivity, the number of red triangles shown on the screen will change.
...
In our case we will need to leave the default **Static Threshold** for this section as we do not have enough historical data.
...
Scroll down further to the **Alert Channel**. Here we can select the Email Alert Channel that you created in the previous section.

Select the **Select Alert Channels** button.

![add-mq-alert-channel](images/add-mq-alert-channel.png)

A dialog will open with the Alert Channel that you created earlier. Select the channel.
...
application. You'll see a notification letting you know your Smart Alert has been created.

![](images/qotd-smart-alert-notification.png)

To see the **Smart Alert** that you just created, click on the
...
You will see a list of 300+ Events that are defined as out of the box thresholds within Instana. If you want to filter the
...
Select the dropdown and choose **Built-in Metrics**. Most of your thresholds will be written against these **Built-in Metrics**.
...
Select the dropdown list. In the dropdown list, you'll see the following options:
...
You can use the **Mail** bookmark to view alert emails that are sent for the alerts you have setup.
