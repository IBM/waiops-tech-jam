...
Begin by configuring Instana to support SMTP email. From the Bastion host
(Guacamole) open a terminal window by selecting **Activities** at the top left
of the screen and then the terminal icon.

![open-terminal](images/open-terminal.png)

Connect to the Instana server by running the following command:

> Type _yes_ when prompted to accept the host key.

```sh
ssh jammer@instana
```

Next, become the root user by running the following command:

```sh
sudo su -
```

Now you can configure Instana to use the locally installed **Postfix** email
server.

We will be using the **stanctl backend apply** command to setup the smtp configuration.  
This command can be used to update many parameters within
your Instana server.  To see a full list of options, type **stanctl backend apply --help**

```sh
stanctl backend apply --help
```

Now, update the smtp settings by entering the following command.

```sh
stanctl backend apply \
  --core-smtp-from="server@instana.com" \
  --core-smtp-host="instana.techzone.lan" \
  --core-smtp-port="25" \
  --core-smtp-user="jammer" \
...
Instana will now apply the config and restart the necessary pods. From the same
terminal windows you can run the following command to see the progress:
...
Give Instana few minutes to apply the configuration.

To configure an Aler Channel navigate back to Firefox and begin by selecting the
**Settings** gear in the left side navigation. Then, select **Alert Channel**
...
Check for the test email on the Instana server. Navigate back to your terminal
window byb selecting **Activities** at the top left of the screen and then the
**terminal**.

As you are currently the _root_ user you will need to run the following command
to switch to the _jammer_ user:

```sh
su - jammer
```

Once logged in use the `mail` command as presented below to view the jammer
users inbox:
...
![](images/thresholds-options-default-static.png)
...
Below there is also an option to select an **Alert Channel**. In the previous
...
Now, scroll down a little further to fill in the remaining options.
...
![](images/alert-events-section.png)
