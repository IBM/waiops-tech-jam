...
**IMPORTANT**:Before starting this tutorial, Please enable Impact Service as
follows:
...
Copy the public key to ~/.ssh/authorized_keys for jammer user on the remote host
(control node) where we want to execute the command.

Click on "Save", go back to "Automation type", the "Script" status should be
"Connected".

**IMPORTTANT** : When you add the generated public key to your control node, be
very careful not to edit the configured public key or you will lock yourself out
