...
**NOTE:** A full list of the topology Observers are documented here:
...
**NOTE: If you can't find `Topology Configuration` option from the UI, please
ensure you have enable the roles in
[2.7 Locate your login details and log in to Watson for AIOps Event Manager](https://ibm.github.io/waiops-tech-jam/tutorials/aiops/installation/#2-7:-locate-your-login-details-and-log-in-to-watson-for-aiops-event-manager)**
...
**NOTE:** There are two resources in the list called "computer1000". One is from
the topology segment ingested from the infrastructure.txt file and the other one
is from the topology segment ingested from the application.txt file. We will be
merging these two Resources together shortly, via a Merge Rule.

Click on router1 and it will open the topology view. Increase the number of hops
to 2 and click Render and you should see something like the following:
...
**NOTE:** It is good practice to specify an appropriate set of Conditions for
your rules, to ensure they only get applied to things they're meant to.
...
you would have seen two. Click on computer1000, change the hop view to 2 and
click Render. You should see something like the following, showing the combined
topology segments:
