...
**NOTE:** The Priority specifies the order in which the Policies will be tried.
Once a scope-based event grouping Policy matches and the scope is set for an
event, all subsequent Policies will be skipped. If two policies have the same
Priority value (as in this case), the execution order is indeterminate.
...
**NOTE:** You may need to wait a few seconds and refresh the event view before
the grouping is visible.
