...
f) Please Note : You need to delete 1 network policy in cluster in order to
successfully run next curl command that connects to ObjectServer REST API
...
Output: { "rowset": { "osname": "AGG_P", "affectedRows": 1, "coldesc": [{
...
Replace the '\<HOSTNAME\>' with the Control Node eth0 IP address captured in
11-5 in below command.
