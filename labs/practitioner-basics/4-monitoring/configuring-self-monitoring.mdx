...
1. Rate of events ingested by lifecycle over time:
   ```
   sum(irate((flink_taskmanager_job_task_operator_KafkaSourceReader_topic_partition_currentOffset{topic="cp4aiops_cartridge_lifecycle_input_events"}>0)[1m:30s]))
   ```
2. Rate of alert update messages processed by lifecycle over time:
   ```
   sum(irate((flink_taskmanager_job_task_operator_KafkaSourceReader_topic_partition_currentOffset{topic="cp4aiops_cartridge_lifecycle_input_alerts"}>0)[1m:30s]))
   ```
