...
<AnchorLinks>
  ...
  <AnchorLink>3-6: Ingesting the CSV data for machine learning</AnchorLink>
</AnchorLinks>
...
This lab will be showing you how to consume data from CSV files. There are CSV files located in the /home/scadmin/BookInfoDemo/data/bookinfo/data directory. These files contain metrics for Application response time (APP), CPU utilization (CPU), Database transaction volume (DB), and disk utilization (DISKBUSY). This data spans a time period of 17 days.
...
## 3-2: **Creating a topic**
  ...
  To create a topic, open a terminal window and run the following command:
...
[https://www.ibm.com/docs/en/oapi/1.3.6?topic=SSJQQ3\_1.3.6/com.ibm.scapi.doc/admin\_guide/r\_tasp\_csvrules.html](https://www.ibm.com/docs/en/oapi/1.3.6?topic=SSJQQ3_1.3.6/com.ibm.scapi.doc/admin_guide/r_tasp_csvrules.html)
...
When ingesting data from the past, it's important to configure the topic such that it doesn't automatically delete the data you ingest. Metric Manager is not meant to be used as a long-term metric data repository... it is meant to keep the data long enough to successfully identify changes in behavior, and to keep a small amount of historical for short-term review. When you create a topic, metric data is cleared out after 15 days, while anomaly alarm data is kept for 180 days. Since we have data originating from 2020, we need to set a couple more topic parameters to ensure the system doesn't age out our data. Set the following:
...
Another thing you may need to do during a proof of concept is disable the alarm clearing. This is required to be disabled if you are analyzing past data and need to demonstrate what the anomaly alerts that arrive in Event Manager look like, without having them clear when the anomaly occurrence stops. Since we'll be viewing Event Manager events as part of our labs, we'll disable it:
...
## 3-4: **Creating a model to ingest CSV**
 ...
 The Mediation Tool is the component you will use to model the data in the CSV files so that Metric Manager can ingest, process, learn, and identify anomalies within the data. The tool allows you to create a 'model' of the data that resides in the CSV files you'll be working with, from within a user-friendly Eclipse interface. You can start the Mediation Tool by issuing the following commands from your VNC session terminal:
...
```
 ...
 You will be presented with the initial config screen. Just leave the default and click "Ok".

 ![](images/Picture2.png)


 You will be presented with the main window. In this step we will create a new Predictive Insights project. From the main window select **File-\>New-\>Project** from the menu bar:
...
### Create a data source
 ...
 In the next screen, you will select **Filesystem**, and click on **Metric names in column headers**.
...
_A note about CSV structure: CSV files can be in either **wide** or **skinny** format. **Wide** format specifies the metric name in the CSV header. For example, in our data the CPU group has the following header:
 ...
 Above we can see there are two metric definitions in the csv header, "User%" and "Wait%". That means that a single line contains multiple metric definitions per resource. Hence, we selected "Metric names in column headers".

 You may come across a situation where the metric name appears in the csv file data rather than the header. For example:
...
**/home/scadmin/BookInfoDemo/data/bookinfo/data**
 ...
 Next, modify the name pattern. As we learned previously, a CSV file name must start with a group refence, followed by a delimiter, followed next by a start time and optionally an end time. When defining the name pattern, we use regular expressions to match. To keep it simple, use the following regex to define the name pattern:

**(.\*)\_\_(.\*)\_\_(.\*)\.csv**

 Note that the literal underscore delimiters are not a single underscore, but two.
...
The files provided are in GMT already so we can leave the timezone as it is.
 ...
 _Note: It is important to be cognizant of the timezone of the data and configure this appropriately, so that the analytics can properly learn the differences between weekday and weekend behavior patterns, as well as proper correlation of metrics across all CSV files. Much of the time the timezone will be GMT, but there are also several cases where they are provided in the local time zone. Make sure that you verify with the customer that the time zone is GMT, and if not, verify the actual time zone._
...
### Defining the model from the data
 ...
 Expanding the tree view fully shows all of the available tables in our CSV files.:
 ...
 As noted earlier, the time format is a bit different in the file that it is in the filename. As such we must define **Time Format** as **dd/MM/yy HH:mm** (don't forget the space between the date and time). When you are finished entering the Time Format field, hit the **enter** key so that the entry is saved.
 ...
 Finally, take a moment to save the work we have done so far for modelling the csv data in case we should lose connection or some other problem. In the Menu, select:
...
You'll notice that indeed there are multiple entries for a given time stamp.
 ...
 Data quality is paramount to ensure maximum value from Metric Manager. Be cognizant of the format of the data, and if there is something you come across that doesn't seem right, consult the owners of the tool that's generating the data for Metric Manager and verify that it is correct..
...
`<CHECK "Save as Project Defaults">`  
 ...
 In this section, we went through the steps to model data that is in CSV format, and deploy that model to a topic. In the next section, we will ingest the data and see what anomalies were identified.
...
### Verify required services are running
 ...
 Before we start the extraction, verify that the Netcool services are up and running by issuing the following command, using the password \<LAB PASSWORD\> as the password:
...
```
 ...
 Take a break. In about 12 to 15 minutes, the data directory will be empty of all CSV files, which have been moved to the good directory. Machine learning and anomaly detection of the ingested data will complete. The next lab will walk through navigating the UI and the results.
