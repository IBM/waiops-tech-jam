...
<AnchorLinks>
  <AnchorLink>2-1: Instana Agent Deployment Options</AnchorLink>
  <AnchorLink>2-2: Installing Instana agent on Linux VM</AnchorLink>
  <AnchorLink>2-3: Installing Instana Agent on the Instana Server</AnchorLink>
  <AnchorLink>2-4: Installing Instana agent on OpenShift cluster</AnchorLink>
  <AnchorLink>2-5: Troubleshooting Instana agent installation on Kubernetes cluster</AnchorLink>
  <AnchorLink>2-6: Configuring Proxy</AnchorLink>
  <AnchorLink>2-7: Configuring Instana Agent</AnchorLink>
  <AnchorLink>2-8: Agent Configuration Troubleshooting</AnchorLink>
  <AnchorLink>2-9: Configuring GitOps for Agents</AnchorLink>
</AnchorLinks>

## 2-1:  Instana Agent Deployment Options
...
## 2-2:  Installing Instana Agent on Linux VM

You will be installing the Instana Agent on the ACE/MQ server that has been provisioned for you for this lab.  Open up a terminal window and login to the ACE/MQ server as root.
...
## 2-3:  Installing Instana Agent on the Instana Server

Next, you will install the Instana agent on the Instana server.  Open a terminal window on the Instana Server.  Then, type **sudo -i** to switch to the root user.
```sh
sudo -i
```
...
## 2-4:  Installing Instana Agent on OpenShift Cluster

The next step is to install the Instana Monitoring into the OpenShift Cluster that we'll be using to install our Bee-Travels application.  This is the cluster that we'll be monitoring.
...
To install Instana agent on OpenShift cluster you need to authenticate to the OCP cluster with appropriate permissions (cluster admin role). Then, follow the steps below:
...
## 2-5:  Troubleshooting Instana Agent Installation on Kubernetes Cluster
...
## 2-8:  Agent Configuration Troubleshooting
...
## 2-9:  Configuring GitOps for Agents
...
Proceed to section 3, where you will install and instrument the Bee-Travels application.