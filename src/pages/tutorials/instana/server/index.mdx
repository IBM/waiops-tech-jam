...
<AnchorLinks>
  <AnchorLink>1-1: Introduction</AnchorLink>
  <AnchorLink>1-2: Prerequisites</AnchorLink>
  <AnchorLink>1-3: Install Instana Server</AnchorLink>
  <AnchorLink>1-4: Activate the License</AnchorLink>
  <AnchorLink>1-5: Launch the Instana User Interface</AnchorLink>
  <AnchorLink>1-6: Turning on features and setting up SMTP</AnchorLink>
  <AnchorLink>1-7: Upgrading the Instana Server</AnchorLink>
  <AnchorLink>1-8: Summary</AnchorLink>
</AnchorLinks>
...
- Large scale kubernetes deployments run the cloud native portion in kubernetes and the datastores distributed across multiple servers.  For assistance with architecture/planning, 
  contact the SWAT team or open a [Grow My Deal](https://ibm.biz/grow-my-deal) request.
...
The instructions can be found here for your reference, but you can follow the steps below.  (https://www.ibm.com/docs/en/instana-observability/current?topic=instana-self-hosted-backend-docker-premises).  
...
Login to virtual machine that was assigned to you for the installations of the Instana Server.  Use the credentials provided in the slack channel.
...
When you run the **instana init** command, it generates a file named settings.hcl in your current directory.  It is important to save this file.  The settings.hcl file contains
the configuration settings for your deployment.  It can be used to enable additional features and make other configuration settings in the future.
...
## 1-5:  Launch the Instana User interface
...
Enter an HTTPS URL with the fully qualified hostname of your server.  Example:   https://hostname1.iops.tech-jam.com
...
## 1-6:  Turning on features and setting up SMTP
...
1. Edit `settings.hcl` file according to the [documentation](https://www.ibm.com/docs/en/obi/current?topic=installer-enabling-feature-flags). Some common feature flags available are:
...
## 1-7:  Upgrading the Instana server
...
You are now ready to continue with other portions of the lab.    Navigate to section 2 of the Instana lab.
