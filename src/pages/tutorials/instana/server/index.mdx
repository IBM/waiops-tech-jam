...
For the lab, you were assigned a virtual machine labeled the **control node**.  ssh into the control using the id_rsa file that you previously downloaded as shown below:
```sh
...
apt-get install docker-ce docker-ce-cli containerd.io docker-compose-plugin -y
 ...
 Note:  There is a **Show more** button on the dialog.  Make sure you click the **Show more** button to see all of the commands.
  ![](images/prereqs.png)

Make sure you copy all of the commands.  After the commands have executed, verify that docker is installed and running.  You can issue **docker ps** command to confirm that it is installed and running.  If docker is running, you should see output similar to what's shown below.
![](images/docker.png)
...
Next, create the following directories and/or filesystems that will be used for the Instana datastores (/mnt/metrics, /mnt/traces, /mnt/data).  Ideally, these directories/filesystems should be setup on fast storage volumes because the datastores are very I/O intensive.
```sh
mkdir /mnt/metrics
mkdir /mnt/traces
mkdir /mnt/data
...
Press **Enter** and let Instana generate a self-signed certificate.  If you were deploying at a customer site and the customer wanted to user CA signed certificates, this is the step where you would specify the existing certificat.
