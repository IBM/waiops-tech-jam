...
6) The operator will install and when it's ready you can view the operator (Click on `View Operator`) to then create an instance, which will be your KubeTurbo probe that will monitor and manage this cluster.
...
3) Before we can fill in the YAML, we need to get the turbonomic server URL
