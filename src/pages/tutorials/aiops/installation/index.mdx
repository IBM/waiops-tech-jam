...
- Establish a connection to your control virtual machine using the supplied SSH key
...
Prior to starting this lab, you should have received two emails: one containing the IP address of your "control" virtual machine, and one containing a zip file with an id_rsa private and public key, which you will need to access the control VM.

- Follow the instructions in the email to unzip the id_rsa.zip file using the supplied password and save the key files onto your laptop
- Open a command terminal (on Windows, use "Windows Powershell")
- In the terminal session, navigate to the directory where your rsa_id file is located
- Open an SSH session as the "jammer" user to your control box using the IP address provided to you in the other email - example:

```sh
ssh -i rsa_id jammer@10.20.30.40
```

After you have successfully opened a terminal session to your control virtual machine, switch to your Internet browser and log in to [https://cloud.ibm.com](https://cloud.ibm.com), and select the account labelled **2576490 - itztsgltechlabs01** from the top-right drop-down menu. Next, click on the menu button in the top-left corner, select **OpenShift**, then **Clusters**.
...
Now go back to your terminal session you opened earlier (to your control virtual machine), and paste the full "oc login" command and press Enter. This command will make a connection to your cluster and authenticate you, so you will be ready to start preparing the cluster by issuing various commands against it.
...
**NOTE:** It is important to know which project/namespace you are in when issuing `oc` commands. All of the subsequent commands need to be run in the `noi` project/namespace otherwise the various elements may not be available to use in later steps.
...
You are now ready to install the Cloud Pak for Watson AIOps Event Manager Operator and deploy an instance.
...
**NOTE:** It is essential to ensure the indentation is correct.

On the main left menu, navigate to **Administration > Cluster Settings**. Under **Global Configuration > OperatorHub > Sources**, verify that the `ibm-operator-catalog` CatalogSource object is present.

**NOTE:** It can take a few minutes to pull down the entire catalogue list, but you should end up with 221 in the **# of Operators** column and show a READY status.
...
Installation of the Operator will take a few minutes as it pulls down the installation packages from the IBM software repository.
...
This final step involves deploying a running instance of Event Manager onto the OpenShift cluster.

Before deploying an instance of Event Manager, you will need to know your cluster's **Ingress subdomain**. To obtain this, return to your IBM Cloud browser tab where you have the "manage-from" cluster details open, and scroll down to the **Networking** section. You will see a box labelled **Ingress subdomain** that contains the domain value for your cluster:
...
* Click on the menu button in the top-left corner of the screen, and select **Netcool WebGUI** from the drop-down menu. A new tab will open with the Netcool/WebGUI UI displayed.
