...
Click on your "manage-from" OpenShift cluster, then click the blue **OpenShift web console** button to open the main OpenShift UI:
...
Go to your OpenShift console and navigate to `Networking -> Routes`, select `noi` project and search for `evtmanager-ibm-hdm-common-ui`. Click on the `Location` link to open up the Event Manager:

![](images/evtmgr_access_url.png) 
