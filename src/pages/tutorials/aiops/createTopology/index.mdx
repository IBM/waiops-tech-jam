...
* Perform topology-based event correlation for events occurring within a group of resources

The topology capability can ingest topology data from many different sources. The multiple disparate pieces of topology can then be stitched together, where it makes sense to do so, allowing you to see your environment with all the logical and physical connections represented.
...
The next step involves copying the two files to the pod running the File Observer in your environment.

First, open a command prompt and authenticate to your OpenShift cluster. The steps to do this are covered in step 2 of module 1 in this series.

Next, using the following as an example, identify the name of the pod running your File Observer, then use the oc cp command to copy the two files to the directory shown below:



```sh
oc get pods | grep file
evtmanager-topology-file-observer-7f984cc888-bf2vm 1/1 Running 0 17d

oc cp application.txt evtmanager-topology-file-observer-7f984cc888-bf2vm:/opt/ibm/netcool/asm/data/file-observer/.
oc cp infrastructure.txt evtmanager-topology-file-observer-7f984cc888-bf2vm:/opt/ibm/netcool/asm/data/file-observer/.

```

**NOTE:** This method of uploading the file to the pod requires the oc utility is installed on your source system and that you know the name of your pod instance. If you want to automate the loading and reloading of this file to the File Observer job, then you might also consider using curl to create and update the job instead. This method does not require the oc utility, nor does it rely on the name of the pod. Instructions for doing this can be found here:

https://medium.com/ibm-watson-aiops/working-with-file-observers-in-waiops-3-1-1-ca76c257cc46

Now that the two files are copied to the File Observer pod, you are ready to create File Observer jobs that read and process them.
...
Input a label to call your new job - eg. Infrastructure - and type in the name of the file you just copied to the pod - ie. infrastructure.txt - and click Save:
...
Navigate to **Administration → Rules** and click on New to create a new Merge Rule.
