...
f) Test REST interface, OMNIbus password and port from the step (e) and step (d) are used to test the REST interface using curl. 

```sh
curl -k -u root:<OMNIBUS password from step e> -X POST -d "{ \"sqlcmd\": \"select count(*) from alerts.status\" }" -H 'Accept: application/json' -H 'Content-Type: application/json' http://<ClusterHostname>:30100/objectserver/restapi/sql/factory
```
...
Check status of httpd service on OCP Infra node. If it is not running start it.
...
Now stop httpd Service and insert test alert for httpd Service. (In live monitoring this alert is automatically generated when Service goes down)
...
```sh
oc exec -it noi-ncoprimary-0 -- /bin/bash -c '/opt/IBM/tivoli/netcool/omnibus/bin/nco_sql -server AGG_P -user root -pass $OMNIBUS_ROOT_PWD'
```
...
Connect to OCP Infra node & check status of httpd service.
